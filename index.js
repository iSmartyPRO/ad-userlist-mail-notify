...
function getUsers() {
    return new Promise((resolve, reject) => {
        var adConfig = {
            ...
            attributes: {
                user: ['sAMAccountName', 'description', 'mail', 'dn', 'name', 'employeeNumber', 'title'],
                group: []
            }
        }
        ...
        ad.findUsers(query, false, function(err, users) {
            ...
            users.forEach(user => {
                for (branch in branches) {
                    if (user.dn == `CN=${user.name},OU=Users,OU=${branch},${config.baseDN}`) {
...
cron.schedule(config.cronJob, function() {
    getUsers()
        .then(d => {
            let branches = config.branches
            for (branch in branches) {
                sendMail(branches[branch].notificationEmail, branches[branch].description, branches[branch].data)
